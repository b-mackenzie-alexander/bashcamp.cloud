...
if ! id backup &>/dev/null; then
  useradd -m -s /sbin/nologin -c "Backup Service Account" backup
fi
...
if ! id nginx_svc &>/dev/null; then
  useradd -m -s /sbin/nologin -c "Web Server Service Account" nginx_svc
fi
...
echo "sandbox-rocky provision complete"
